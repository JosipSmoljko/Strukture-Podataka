...
int main(){
	...
	pronadiDrzavu(listaDrzava, ime, number);

    return 0;

}

TPosition pronadiGrad(TPosition current, int number)
{
	  if (current == NULL)
		    return NULL;
...
int pronadiDrzavu(LPosition head, char* country, int number)
{
	  ...
	  if (temp)
	  {
		  ...
		  temp->gradovi = pronadiGrad(temp->gradovi, number);
	}
