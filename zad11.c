...
int InsertInListFromFile(LPosition head, char *fileName, list hashTablica[]);
...
int initializeList(LPosition head);
int findCountry(list HashTablica[], LPosition head, char* country, int number);
...
int main(){
    ...
    InsertInListFromFile(listaDrzava, drzave, &HashTablica);
	...
	findCountry(HashTablica, listaDrzava, ime, number);
...
int HashTable(list HashTab[],TPosition newEl)
{
	int num = HashNum(newEl->grad);

	SortListWhileInserting(newEl,&HashTab[num]);
